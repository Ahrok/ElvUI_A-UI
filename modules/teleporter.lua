...
-- =====================================================================
-- 1. DATENBANK (Kategorien & Items nach Spielversion)
-- =====================================================================
local TeleportList = {}

if isRetail then
    -- -----------------------------------------------------------------
    -- RETAIL / MIDNIGHT SETUP
    -- -----------------------------------------------------------------
    TeleportList = {
        {
            ...
            items = {
                ...
                { id = 210455, type = "toy", races = {"Draenei", "LightforgedDraenei"} },   -- Draenischer Holostein
                { id = 183716, type = "toy" },   -- Venthyr-Sündenstein
                { id = 265100, type = "toy" },   -- Ruhestein der Kernwächterin
                { id = 263933, type = "toy" },   -- Ruhestein des Beutesuchers
            }
        },
        {
            ...
            items = {
                ...
                { id = 18986, type = "item" },   -- Ultrasicherer Transporter: Everlook
    ...
    TeleportList = {
        {
            name = L["Hearthstones"] or "Ruhesteine & Items",
            icon = "Interface\\Icons\\INV_Misc_Rune_01",
            items = {
                { id = 6948, type = "item" },    -- Klassischer Ruhestein
                { id = 28585, type = "item" },   -- Rubinschuhe (Karazhan)
                { id = 32757, type = "item" },   -- Medaillon von Karabor (Teleport zum Schwarzen Tempel)
            }
        },
        {
            name = L["Engineering"] or "Ingenieurskunst & Portale",
            icon = "Interface\\Icons\\Trade_Engineering",
            items = {
                { id = 30542, type = "item" },   -- Dimensionszerfetzer: Area 52
                { id = 30544, type = "item" },   -- Ultrasicherer Transporter: Toshleys Station
                { id = 18986, type = "item" },   -- Dimensionszerfetzer: Everlook
                { id = 18984, type = "item" },   -- Ultrasicherer Transporter: Gadgetzan
                { id = 22589, type = "item" },   -- Atiesh, Hohestab des Wächters (Karazhan Portal)
                { id = 22630, type = "item" },   -- Atiesh (Druide)
                { id = 22631, type = "item" },   -- Atiesh (Priester)
                { id = 22632, type = "item" },   -- Atiesh (Hexenmeister)
            }
        },
        {
            name = L["Druid"] or "Druide",
        ...
        },
        {
            ...
            items = {
                ...
                { id = 3566, type = "spell" },  -- TB
                { id = 32272, type = "spell" }, -- Silbermond
                { id = 33690, type = "spell" }, -- Shattrath (Allianz)
                { id = 35715, type = "spell" }, -- Shattrath (Horde)
                { id = 49359, type = "spell" }, -- Theramore
                { id = 49358, type = "spell" }, -- Steinard
            }
        },
        {
            name = L["Mage Portals"] or "Magier Portale",
            ...
            items = {
                ...
                { id = 11420, type = "spell" }, -- TB
                { id = 32267, type = "spell" }, -- Silbermond
                { id = 33691, type = "spell" }, -- Shattrath (Allianz)
                { id = 35717, type = "spell" }, -- Shattrath (Horde)
                { id = 49361, type = "spell" }, -- Theramore
                { id = 49360, type = "spell" }, -- Steinard
            }
        }
    }
end
...
local function GetSafeName(id, itemType)
    local name, icon
    if itemType == "toy" and isRetail then
        name, _, _, _, _, _, _, _, _, icon = GetItemInfo(id)
        if not name and C_ToyBox and C_ToyBox.GetToyInfo then
            local _, tName, tIcon = C_ToyBox.GetToyInfo(id)
            name, icon = tName, tIcon
        end
    elseif itemType == "item" or itemType == "toy" then
        name, _, _, _, _, _, _, _, _, icon = GetItemInfo(id)
        ...
        if not category.class or category.class == E.myclass then
            ...
            for _, itemData in ipairs(category.items) do
                ...
                if isRaceAllowed then
                    if isRetail and itemData.type == "toy" and PlayerHasToy and PlayerHasToy(id) and C_ToyBox and C_ToyBox.IsToyUsable and C_ToyBox.IsToyUsable(id) then
                    ...
                    elseif itemData.type == "spell" and (IsSpellKnown(id) or (IsPlayerSpell and IsPlayerSpell(id))) then
...
preloadFrame:SetScript("OnEvent", function(self, event)
    for _, category in ipairs(TeleportList) do
        for _, itemData in ipairs(category.items) do
            ...
            for _, category in ipairs(TeleportList) do
                if not category.class or category.class == E.myclass then
                    for _, itemData in ipairs(category.items) do
                        ...
                        if isRaceAllowed then
                            local owns = false
                            if isRetail and itemData.type == "toy" and PlayerHasToy and PlayerHasToy(itemData.id) and C_ToyBox and C_ToyBox.IsToyUsable and C_ToyBox.IsToyUsable(itemData.id) then 
                                owns = true
                            elseif itemData.type == "item" and GetItemCount(itemData.id) > 0 then 
                                owns = true
                            elseif itemData.type == "spell" and (IsSpellKnown(itemData.id) or (IsPlayerSpell and IsPlayerSpell(itemData.id))) then 
                                owns = true 
                            end
